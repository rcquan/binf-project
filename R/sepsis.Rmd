...
centerScale <- function(dtm) {
	toScale <- dtm %>% dplyr::select(-threadID, -label) %>% scale()
	dtm %<>% dplyr::select(threadID, label) %>% 
		cbind(., toScale)
	return(dtm)
}
```
...
createTrainTest <- function(featureSet) {
	trainIndex <- createDataPartition(featureSet$label, times=1, p=0.7)$Resample1
	fTrain <<- featureSet[trainIndex, ]
	fTest <<- featureSet[-trainIndex, ]
}
```
...
crossValidate <- function(trainSet) {
	...
	nlpLogModel <- train(as.factor(label) ~ ., data=trainSet, trControl=tc,  
						 method="glm", metric="ROC")  
	
	nbGrid <- expand.grid(.fL=0, .usekernel=F)
	nlpNaiveBayes <- train(as.factor(label) ~ ., data=trainSet, trControl=tc, 
						   method="nb", tuneGrid=nbGrid, metric="ROC")  
	
	infoGrid <- expand.grid(.C=0.25)
	nlpInfoModel <- train(as.factor(label) ~ ., data=trainSet, trControl=tc, 
						  method="J48", tuneGrid=infoGrid, metric="ROC") 
	
	rpartGrid <- expand.grid(.cp=0.2)
	nlpGiniModel <- train(as.factor(label) ~ ., data=trainSet, trControl=tc, 
